...
# Initialize a dictionary to keep track of the unique faces
unique_faces = {}
    ...
    while True:
      ...
      if results.detections:
         for id, detection in enumerate(results.detections):
            ...
            found_match = False
            for face_id, face_data in unique_faces.items():
                # Calculate the distance between the current face and the previous face
                distance = abs(bbox[0] - face_data['bbox'][0]) + abs(bbox[1] - face_data['bbox'][1]) + \
                           abs(bbox[2] - face_data['bbox'][2]) + abs(bbox[3] - face_data['bbox'][3])
                if distance < 100:
                    # This face matches a previously detected face
                    unique_faces[face_id]['bbox'] = bbox
                    found_match = True
                    print(f"Welcome back, visitor {face_id}! This is your {unique_faces[face_id]['visits']} visit.")
                    break
            
            if not found_match:
                # This is a new face
                unique_faces[len(unique_faces) + 1] = {'bbox': bbox, 'visits': 1}
